...
const DrobDownMore = () => {
    const DropItem = ({ onClickEvent, Icon, Title, Toggle }) => {
        return (
            <Link to='' onClick={() => { dispatch(FeaturesAction.ShowDrobdownMore(true)); onClickEvent() }} className='flex gap-3 items-center justify-between px-3 rounded-lg hover:bg-gray-200'>
    ...
    return (
        <>
            ...
            <div className='absolute z-30 top-[4rem] left-2 shadow-gray-200 drop-shadow-lg rounded-lg bg-[rgb(255,255,255,.9)] backdrop-blur-xl text-md font-semibold py-3 px-2 w-[19rem]'>
                ...
                <DropItem Icon={<BsGear />} Title={'Settings'} Toggle={<BsToggleOff />} onClickEvent={() => dispatch(FeaturesAction.setIsSettingsWin(true))} />
